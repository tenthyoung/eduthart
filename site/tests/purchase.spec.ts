 ...
 */
test("buys an original from the artwork page through to the invoice", async ({ page }) => {
  ...
  await expect(page.getByRole("option", { name: /18 Harbour Road/ })).toBeAttached();
  ...
  await page.goto(artwork.href);
  await expect(page.getByRole("button", { name: "Currently unavailable" })).toBeVisible();
...
test("tells a collector when artwork they saved is sold to someone else", async ({ page }) => {
  const artist = await createAccount(page, {
    displayName: "Marina Vale",
  ...
  const watcher = await createAccount(page, { displayName: "Sam Watcher", uid: "collector-watcher" });
  const buyer = await createAccount(page, { displayName: "Robin Buyer", uid: "collector-rival" });
  ...
  await signInAs(page, watcher);
  await page.goto("/notifications");
  await expect(page.getByRole("heading", { name: "An artwork you saved has sold" })).toBeVisible();
});
