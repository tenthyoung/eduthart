 ...
 */
export function buildArtworkKey(reference: Pick<ArtworkReference, "artistUid" | "itemId">) {
  return `${reference.artistUid}__${reference.itemId}`;
}

export function parseArtworkKey(key: string): { artistUid: string; itemId: string } | null {
