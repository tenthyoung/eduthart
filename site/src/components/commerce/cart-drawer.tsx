        ...
        <SheetHeader className="border-b border-border px-6 py-5">
          ...
          <SheetDescription>
            {items.length === 0
              ...
              : `${items.length} original ${items.length === 1 ? "artwork" : "artworks"} selected.`}
          </SheetDescription>
        </SheetHeader>
          ...
          {status !== "authenticated" ? (
            <div className="rounded-2xl border border-dashed border-border p-8 text-center">
              <p className="text-sm leading-6 text-muted-foreground">
                Sign in to view your saved cart and begin checkout.
              </p>
              <Button asChild className="mt-5">
                <Link href="/login?next=/cart" onClick={() => setOpen(false)}>
                  Sign in
                </Link>
              </Button>
            </div>
            ...
            <div className="py-16 text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Browse an artist gallery to find an original piece.
              </p>
            </div>
          ) : (
            <div className="space-y-5">
              {items.map((item) => (
                <article
                  key={item.itemId}
                  className="flex gap-4 border-b border-border/70 pb-5"
                >
        ...
        <SheetFooter className="border-t border-border bg-background px-6 py-5">
          {items.length > 0 ? (
            <>
              ...
              <p className="mb-3 text-left text-xs leading-5 text-muted-foreground">
